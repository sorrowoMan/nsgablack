...
- 用一个统一的 `BlackBoxProblem` 抽象，把“目标函数 + 约束 + 变量边界”打包起来，便于在不同优化算法之间复用；
- 提供 NSGA-II 与 VNS 等多种优化器，实现从全局多目标搜索到局部多邻域搜索的完整链路；
- 同时支持**可视化交互**（调试、教学）和**无界面 headless 运行**（批量实验、服务器部署、云端任务）；
- 借助历史解、多样性初始化、精英管理、ML 模型与降维工具，在复杂工程问题上逐步“积累经验”、提升搜索效率；
- 通过并行批量运行、元优化与可选 Numba 加速，在保证易用性的前提下尽可能挖掘 CPU 性能。
...
在当前仓库根目录下，推荐使用 Conda / venv 创建环境，并安装 `requirements.txt`：

```bash
pip install -r requirements.txt
```

（如果你只把 `nsgablack` 文件夹复制到自己的项目，只需要确保 `numpy`, `scipy`, `matplotlib`, `scikit-learn` 等依赖已安装。）
...
solver.start_animation()  # GUI 中也可以通过按钮控制
```
...
下面将主要类和函数的参数用 Markdown 表格列出，包含类型、默认值与说明，便于直接查阅。
...
|       `diversity_params` |      dict      | see README | 包含 `candidate_size`, `similarity_threshold`, `rejection_prob`, `sampling_method` |
...
### 4.1 并行批量运行 (`nsgablack/parallel_runs.py`)
...
## 4.2 元优化模块：自动调节 NSGA-II 参数 (`nsgablack/metaopt.py`)

"""Second-order cone programming (SOCP) template."""
...
def solve_socp_template(
    ...
    if "socp_spec_builder" in params and "spec_builder" not in params:
        params["spec_builder"] = params.get("socp_spec_builder")

    spec = resolve_template_spec(
        request,
        params,
        payload,
        payload_builder_keys=("copt_socp_spec_builder", "copt_spec_builder"),
        inline_keys=("c", "A", "rhs", "sense", "lb", "ub", "vtype", "cones"),
    )

    env = cp.Envr()
    model = env.createModel(str(spec.get("name") or "nsgablack_copt_socp"))
    ...
    cones = spec.get("cones") or spec.get("soc_cones") or []
    for cone in cones:
        data = dict(cone or {})
        refs = data.get("vars") or data.get("indices") or data.get("var_refs")
        if refs is None:
            raise ValueError("cone entry requires 'vars' or 'indices'")
        vars_ref = resolve_var_refs(list(refs), vars_list, name_map)
        cone_type = str(data.get("type", data.get("cone_type", "quad"))).strip().lower()
        if cone_type in {"quad", "soc", "socp"}:
            copt_type = cp.COPT.CONE_QUAD
        elif cone_type in {"rquad", "rotated", "rsoc", "rsocp"}:
            copt_type = cp.COPT.CONE_RQUAD
        else:
            raise ValueError(f"unsupported cone type: {cone_type}")
        model.addCone(vars_ref, copt_type)

...
vship  = mtrnloc.addVars(cities, cities, vtype=COPT.INTEGER, nameprefix='ship')

# Add constraints to problem
mtrnloc.addConstrs((vship.sum(i, '*') <= supply[i] * vbuild[i] for i in cities), nameprefix='supply')
mtrnloc.addConstrs((vship.sum('*', j) >= demand[j] for j in cities), nameprefix='demand')
mtrnloc.addConstr(vbuild.sum('*') <= buildlimit, name='limit')

# Set objective function
mtrnloc.setObjective(vship.prod(shipcost), COPT.MINIMIZE)

# Set optimization parameters
mtrnloc.setParam(COPT.Param.TimeLimit, 100)

# Solve the problem
mtrnloc.solve()

# Display solution
if mtrnloc.status == COPT.OPTIMAL or \
   mtrnloc.status in [COPT.TIMEOUT, COPT.NODELIMIT, COPT.INTERRUPTED] and \
   mtrnloc.hasmipsol:
  print('\nBest MIP objective value: {0:.6f}'.format(mtrnloc.objval))

  print('')
  for key in vbuild:
    if vbuild[key].x > 1e-6:
      print('Build warehouse in {0:6s}'.format(key))

  print('')
  for key in vship:
    if vship[key].x > 1e-6:
      print("Ship {0:.0f} items from {1:6s} to {2:6s}".format(vship[key].x, key[0], key[1]))
else:

...
from nsgablack.plugins import BenchmarkHarnessPlugin, ModuleReportPlugin, MonteCarloEvaluationProviderPlugin, MonteCarloEvaluationConfig
...
class NoisyConstrainedProblem:
    ...
    def evaluate_constraints(self, candidate):
        candidate = np.asarray(candidate, dtype=float)
        # constraint: sum(candidate) <= 1
        g = float(np.sum(candidate) - 1.0)
...
def build_solver():
    ...
    solver.set_bias_enabled(True)
    solver.register_evaluation_provider(MonteCarloEvaluationProviderPlugin(MonteCarloEvaluationConfig(mc_samples=12)).create_provider())

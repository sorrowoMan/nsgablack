...
def get_problem_registry() -> ProblemRegistry:
    return ProblemRegistry(
        registry=(
            ProblemSpec(key="pooled_handoff", params={"dimension": 3}),
        )
    )
...
def _build_pooled_problem(cfg: Optional[ProblemConfig] = None) -> PooledHandoffProblem:
    cfg = cfg or ProblemConfig()
    return PooledHandoffProblem(dimension=int(cfg.dimension))
...
def _register_builtin_problems() -> None:
    def _pooled_builder(params: Dict[str, Any]) -> PooledHandoffProblem:
        return _build_with_config(ProblemConfig, params, _build_pooled_problem)

    register_problem_builder("pooled_handoff", _pooled_builder)

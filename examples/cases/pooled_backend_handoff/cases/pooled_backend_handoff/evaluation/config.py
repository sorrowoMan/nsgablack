...
def get_evaluation_registry() -> EvaluationRegistry:
    return EvaluationRegistry(
        registry=(
            EvaluationSpec(key="copt_handoff", params={"threads": 4}),
        )
    )
...
def _register_default_builders() -> None:
    from .copt_provider import CoptHandoffProvider

    register_evaluation_provider_builder("copt_handoff", lambda params: CoptHandoffProvider())

...
from .example_problem import ClassificationThresholdCalibrationProblem
...
def get_problem_registry() -> ProblemRegistry:
    return ProblemRegistry(
        registry=(
            ProblemSpec(key="example", params={"n_samples": 240, "valid_ratio": 0.3, "seed": 13}),
        )
    )
...
def _build_example_problem(cfg: Optional[ProblemConfig] = None) -> ClassificationThresholdCalibrationProblem:
    cfg = cfg or ProblemConfig()
    return ClassificationThresholdCalibrationProblem(
...
def _register_builtin_problems() -> None:
    def _example_builder(params: Dict[str, Any]) -> ClassificationThresholdCalibrationProblem:

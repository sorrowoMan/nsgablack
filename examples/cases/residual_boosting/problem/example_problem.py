# -*- coding: utf-8 -*-
"""Residual boosting case: nsgablack searches recipe parameters, mlblack owns inner training semantics."""
...
class ResidualBoostingProblem(BlackBoxProblem):
    ...
    def evaluate(self, x: np.ndarray) -> np.ndarray:
        key = tuple(np.round(np.asarray(x, dtype=float).reshape(-1), 6))
        ...
        if cached is not None:
            ...
            return cached.copy()
        recipe = self.decode_recipe(np.asarray(x, dtype=float))
        ...
        feedback = SupervisedRegressionProblem(self.data, complexity_weight=0.0).evaluate(
            integrated,
            UnknownState(values=np.asarray(x, dtype=float).reshape(-1), metadata={"recipe": recipe.__dict__}),
            {},
        )
        valid_mse = float(feedback.metrics.get("valid.mse", feedback.objectives[0]))

...
from .example_problem import ResidualBoostingProblem
...
def get_problem_registry() -> ProblemRegistry:
    return ProblemRegistry(
        registry=(
            ProblemSpec(key="example", params={"n_samples": 160, "valid_ratio": 0.25, "seed": 7}),
        )
    )
...
def _build_example_problem(cfg: Optional[ProblemConfig] = None) -> ResidualBoostingProblem:
    cfg = cfg or ProblemConfig()
    return ResidualBoostingProblem(
...
def _register_builtin_problems() -> None:
    def _example_builder(params: Dict[str, Any]) -> ResidualBoostingProblem:

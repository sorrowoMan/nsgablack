...
def build_solver(run_id: str | None = None, *, strategy: str | None = None, quickstart: bool = False, resource_context=None, component_overrides=None):
    ...
    # Dynamic strategy switching: VNS+SA
    from nsgablack.adapters import (
        StrategyRouterAdapter, MultiStrategyConfig, StrategySpec,
        SAConfig, SimulatedAnnealingAdapter, VNSAdapter, VNSConfig,
    )
    config = MultiStrategyConfig(total_batch_size=8, adapt_weights=True)
    strategies = [
        StrategySpec(adapter=VNSAdapter(VNSConfig()), name="vns", weight=0.6),
        StrategySpec(adapter=SimulatedAnnealingAdapter(SAConfig()), name="sa", weight=0.4),
    ]
    solver.set_adapter(StrategyRouterAdapter(strategies=strategies, config=config))

...
from .example_problem import BiObjectiveSphereProblem
...
def get_problem_registry() -> ProblemRegistry:
    return ProblemRegistry(
        registry=(
            ProblemSpec(key="biosphere", params={"dimension": 6}),
        )
    )
...
def _build_biosphere_problem(cfg: Optional[ProblemConfig] = None) -> BiObjectiveSphereProblem:
    cfg = cfg or ProblemConfig()
    return BiObjectiveSphereProblem(dimension=int(cfg.dimension))
...
def _register_builtin_problems() -> None:
    def _biosphere_builder(params: Dict[str, Any]) -> BiObjectiveSphereProblem:
        return _build_with_config(ProblemConfig, params, _build_biosphere_problem)

    register_problem_builder("biosphere", _biosphere_builder)

...
def bench_gmm(data, k, pop_size=50, max_steps=300, seed=42):
    ...
    class GMMProblem(BlackBoxProblem):
        ...
        def evaluate(self, candidate):
            ...
            means = candidate[: k * n_features].reshape(k, n_features)
            log_cov = candidate[k * n_features : 2 * k * n_features].reshape(k, n_features)
            raw_w = candidate[2 * k * n_features : 2 * k * n_features + k]

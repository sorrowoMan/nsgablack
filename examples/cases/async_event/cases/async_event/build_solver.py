...
def build_solver(run_id: str | None = None, *, strategy: str | None = None, quickstart: bool = False, resource_context=None, component_overrides=None):
    ...
    # Async event-driven: SA+VNS with event queue orchestration
    from nsgablack.adapters import (
        AsyncEventDrivenConfig, AsyncEventDrivenAdapter, EventStrategySpec,
        SAConfig, SimulatedAnnealingAdapter, VNSAdapter, VNSConfig,
    )
    from nsgablack.plugins import AsyncEventHubPlugin, ParetoArchivePlugin
    strategies = [
        EventStrategySpec(adapter=SimulatedAnnealingAdapter(SAConfig(batch_size=2, initial_temperature=5.0, cooling_rate=0.95)), name="sa", weight=0.8),
        EventStrategySpec(adapter=VNSAdapter(VNSConfig(batch_size=4, k_max=3, base_sigma=0.2)), name="vns", weight=1.2),
    ]
    config = AsyncEventDrivenConfig(total_batch_size=8, bootstrap_events_per_strategy=4)
    solver.set_adapter(AsyncEventDrivenAdapter(strategies=strategies, config=config))

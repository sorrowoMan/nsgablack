...
from .example_problem import SphereProblem
...
def get_problem_registry() -> ProblemRegistry:
    return ProblemRegistry(
        registry=(
            ProblemSpec(key="sphere", params={"dimension": 8}),
        )
    )
...
def _build_example_problem(cfg: Optional[ProblemConfig] = None) -> SphereProblem:
    cfg = cfg or ProblemConfig()
    return SphereProblem(dimension=int(cfg.dimension))
...
def _register_builtin_problems() -> None:
    def _example_builder(params: Dict[str, Any]) -> SphereProblem:
        return _build_with_config(ProblemConfig, params, _build_example_problem)

    register_problem_builder("sphere", _example_builder)

...
from problem.main import SymbolicKernelDigitsRefinementProblem


class SymbolicKernelDigitsRefinementSolver(ComposableSolver):
...
def build_solver(
    ...
    *,
    ...
    default_config = {
        "refinement_mode": "trust_region_dfo",
        ...
        "inner_device": "cpu",
        "refinement_test_error_weight": 1.0,
        "refinement_gap_weight": 0.75,
        "seed": 42,
    }
    default_bundle = {
    ...
    output_dir = str(overrides.pop("output_dir", "runs/symbolic_kernel_refinement"))
    ...
    if str(cfg.refinement_mode).strip().lower() != "trust_region_dfo":
        raise ValueError("symbolic kernel digits refinement requires trust_region_dfo")
    ...
    solver = SymbolicKernelDigitsRefinementSolver(
        problem=problem,
        adapter=adapter,
...
__all__ = ["SymbolicKernelDigitsRefinementSolver", "build_solver"]

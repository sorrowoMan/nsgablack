...
class SymbolicKernelDigitsRefinementProblem(BlackBoxProblem):
    ...
    def evaluate(self, x: np.ndarray) -> np.ndarray:
        _ensure_mlblack_path()
        from examples.cases.symbolic_kernel_digits_classification.build_solver import (  # type: ignore
            SymbolicKernelDigitsClassificationConfig,
            run_symbolic_kernel_digits_classification,
        )

        candidate = np.asarray(x, dtype=float).reshape(self.dimension)
        ...
        resource_context = lease.resource_context(
            ...
            namespace=f"nsgablack.symbolic_kernel_image_outer_search.{label}",
        )
        inner_cfg = SymbolicKernelDigitsClassificationConfig(
            ...
            mlp_hidden_dims=tuple(int(v) for v in tuple(self.cfg.inner_mlp_hidden_dims)),
            mlp_epochs=int(self.cfg.inner_mlp_epochs),
            mlp_batch_size=int(self.cfg.inner_mlp_batch_size),
            mlp_lr=float(self.cfg.inner_mlp_lr),
            mlp_weight_decay=float(self.cfg.inner_mlp_weight_decay),
            pipeline_mode="symbolic",
        ...
        try:
            result = run_symbolic_kernel_digits_classification(
                inner_cfg,
                suite_id=label,

...
class SymbolicKernelDigitsOuterSearchProblem(BlackBoxProblem):
    def __init__(self, cfg: SymbolicKernelDigitsOuterSearchConfig, *, output_dir: str | Path) -> None:
        ...
        from examples.cases.symbolic_kernel_digits_classification.case_scaffold.pipeline import available_symbolic_kernel_basis  # type: ignore
        from examples.cases.symbolic_kernel_digits_classification.case_scaffold.problem import get_image_classification_dataset_spec  # type: ignore

        self.symbolic_basis_library = tuple(available_symbolic_kernel_basis())
        self.dataset_spec = dict(get_image_classification_dataset_spec(self.cfg.inner_dataset_key))
    ...
    def decode_structure_bundle(self, x: np.ndarray) -> dict[str, Any]:
        _ensure_mlblack_path()
        from examples.cases.symbolic_kernel_digits_classification.case_scaffold.pipeline import build_symbolic_kernel_object  # type: ignore
        ...
        symbolic_kernel_object = build_symbolic_kernel_object(
    ...
    def evaluate(self, x: np.ndarray) -> np.ndarray:
        _ensure_mlblack_path()
        from examples.cases.symbolic_kernel_digits_classification.case_scaffold.pipeline import summarize_symbolic_kernel_object_priors  # type: ignore

        structure_bundle = self.decode_structure_bundle(x)
        ...
        prior_summary = summarize_symbolic_kernel_object_priors(dict(bundle.get("symbolic_kernel_object", {}) or {}))

...
class LOFProblem(BlackBoxProblem):
    ...
    def evaluate(self, x):
        k_neighbors = max(2, int(round(float(x[0]))))
        contamination = float(np.clip(x[1], 0.01, 0.5))
        try:
...
class IsolationForestProblem(BlackBoxProblem):
    ...
    def evaluate(self, x):
        n_estimators = max(10, int(round(float(x[0]))))
        max_samples = float(np.clip(x[1], 0.1, 1.0))
        max_features = float(np.clip(x[2], 0.1, 1.0))
        try:

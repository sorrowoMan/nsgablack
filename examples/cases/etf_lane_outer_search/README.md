...
- mlblack 负责 ETF temporal forecast data/model/evaluation surface。
...
`python examples\cases\etf_lane_outer_search\run_solver.py --suite-id etf_reconnect_smoke_fixed --pop-size 4 --offspring-size 2 --generations 1 --seeds 42 --wf-max-folds 1 --wf-max-train-panel-rows 4000 --wf-max-test-panel-rows 1600 --baseline-models ridge` 已验证 nsgablack 外层能调用新的 mlblack ETF 标准 case：

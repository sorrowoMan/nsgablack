...
from runtime import load_production_case_data, resolve_production_case_dir


def _resolve_default_baseline_plan(base_dir: Path) -> Optional[Path]:
    del base_dir
    cases_dir = resolve_production_case_dir()
    export_dirs = (
        cases_dir,
...
def _load_case_data(base_dir: Path, bom: Optional[str], supply: Optional[str], *, machines: int, materials: int, days: int):
    ...
    del base_dir
    data = load_production_case_data(
        bom_path=bom,
        supply_path=supply,
...
def _build_solver_from_args(args, *, resource_context=None):
    ...
    outer_task_threads = int(max(1, int(args.outer_task_threads)))
    if resource_context is not None:
        grant = coerce_resource_context(resource_context)
        granted_threads = max(1, int(grant.threads))
        requested_outer = outer_workers
        requested_inner = inner_workers
        requested_task_threads = outer_task_threads
        outer_workers = min(outer_workers, granted_threads)
        threads_per_outer = max(1, granted_threads // outer_workers)
        inner_workers = min(inner_workers, threads_per_outer)
        outer_task_threads = min(max(outer_task_threads, inner_workers), threads_per_outer)
        if (outer_workers, inner_workers, outer_task_threads) != (
            requested_outer,
            requested_inner,
            requested_task_threads,
        ):
            print(
                "[l0] nested worker grant applied "
                f"threads={granted_threads} "
                f"outer_workers={requested_outer}->{outer_workers} "
                f"inner_workers={requested_inner}->{inner_workers} "
                f"outer_task_threads={requested_task_threads}->{outer_task_threads}"
            )
    outer_task_requirement = _build_resource_requirement(
        threads=outer_task_threads,
    ...
    problem = SupplyEventShiftProblem(
        ...
        production_case_dir=resolve_production_case_dir(),
    ...
    solver.l0_runtime_summary = {
        ...
        "run_id": str(run_id),
        "project_grant_threads": (
            int(coerce_resource_context(resource_context).threads)
            if resource_context is not None
            else None
        ),
        "outer": {
...
def build_solver(argv: Optional[list] = None, *, resource_context=None):

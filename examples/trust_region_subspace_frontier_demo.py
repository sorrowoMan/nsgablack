...
from nsgablack.utils.plugins import BenchmarkHarnessPlugin, ModuleReportPlugin, SubspaceBasisPlugin, SubspaceBasisConfig
...
def build_solver():
    ...
    adapter = TrustRegionSubspaceAdapter(TrustRegionSubspaceConfig(subspace_dim=6, batch_size=16))
    solver = ComposableSolver(problem=problem, adapter=adapter, representation_pipeline=pipeline)
    solver.add_plugin(SubspaceBasisPlugin(SubspaceBasisConfig(method="pca", subspace_dim=6, min_samples=16)))

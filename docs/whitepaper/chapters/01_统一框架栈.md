...
Adapter 通过 `propose(solver, context)` 和 `update(solver, candidates, feedback, context)` 表达策略，其中正式反馈是 `OptimizationFeedbackBatch`，纯数值算法仍可解包为 objectives / violations；候选通过 CandidateBatch 对齐 UnknownState 语义视图与 ndarray 数值视图。进阶 Adapter 还应实现 state、population 和 context projection 接口，以支持 checkpoint、权威种群提交和审计。Adapter 更新后的 population 是代级快照的首选权威来源。
...
LearningSolver 不是第二套控制平面，而是把 ML 的 DataView、Codec、Problem、Provider、Artifact 与 `fit()`/`TrainerResult` 词汇投影到 nsgablack Solver。propose/evaluate/update 统一由 nsgablack AlgorithmAdapter 驱动。多个具有独立生命周期的学习任务必须作为标准子 Case，通过 BlackBase `CaseStageRunner` 与 `CaseRunRequest` 组合；共享一个生命周期的步骤属于 Problem、Provider 或 Pipeline。完整子 Case 的 setup/finish/error/teardown、结果、Artifact、资源与 lineage 均由公共执行信封闭合。

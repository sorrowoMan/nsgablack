# 第一卷·第十四章　身份、时间与版本：同一个“值”为什么不是同一个状态

第十三章建立了一条不能倒置的关系：UnknownState 经 Representation 成为 Candidate，Candidate 接受评价后产生 Feedback，Feedback 推动策略生成新的权威状态，运行最后才从已提交事实形成 Result。那一章还留下一个更困难的问题：如果两个 Candidate 语义等价，它们是否就是同一次提出？如果两份 Snapshot 内容相同，它们是否代表同一个提交点？如果恢复后的 generation 又从 20 开始，新的第 20 代与旧运行的第 20 代怎样区分？
...
本章的命题是：**框架中的一个状态实例至少由语义内容、实例身份、逻辑位置、解释版本与 lineage 共同确定；只有值相同，不足以把两次运行事实合并。** 这不是要求所有对象都携带一张庞大身份证，也不是要求每个节点保存全部祖先，而是要求每一次跨边界、复用、提交和恢复都能取得完成当前判断所需的最小坐标与可继续展开的稳定引用。

---

## 14.1 身份不是一个名字，而是一组逐层收窄的坐标
...
## 14.2 三种“相同”：内容相同、语义等价与实例相同

第十三章已经说明 values 相等不一定 decode 成同一个 Candidate。本章需要进一步区分三种经常被一个 `==` 混在一起的关系。
...
## 14.3 从 run 到 evaluation：每一层身份在防止什么错误
...
## 14.4 逻辑时间：状态不是“最新的”，而是处于某个已命名阶段
...
## 14.6 Lineage：恢复、嵌套、重试和派生都不是复制粘贴
...
不过，即使只保存引用，lineage 的祖先仍然原则上可以无限展开。一个模型 Artifact 指向训练 run，训练 run 指向数据 Artifact，数据 Artifact 指向采集与清洗任务，采集任务又指向设备校准和外部环境。继续追问每一份记录由谁产生，还会得到新的 run、工具和证明材料。这不是 lineage 设计失败，而是现实因果关系本来就没有一个由软件自动发现的绝对起点。

因此，lineage 协议要固定的是坐标与关系类型，而不是祖先层数。可以把一条最小边理解为：

```text
subject_ref       当前要解释的节点
relation          produced_by / consumed / updated_from / resumed_from / attested_by ...
object_ref        直接上游节点
scope             这条关系支持哪个 claim 或阶段
producer/run      谁在何次运行中声明了这条边
evidence_ref      用什么记录验证这条边
```

上游继续展开时，新增的是同构节点和有类型的边，而不是在当前 Result 中继续发明 `source_of_source_of_source_*` 字段。所谓“维度增加”只应发生在框架发现了新的关系语义、现有 relation 无法无损表达时；单纯增加一层祖先，只会增加图的深度。

每一次对外提交还需要形成一个 lineage cut，也就是当前责任边界承诺直接证明的那一圈关系。cut 内的直接输入、父运行、关键转换与权威提交必须有本地见证；cut 外的祖先通过 `child_manifest_ref`、`artifact_ref`、digest 或外部 attestation 继续连接，并声明采用的 provenance policy、trust roots 与 unresolved assumptions。cut 不是把历史切断，而是把“本层负责证明什么”和“需要去哪里继续问”同时固定下来。

这会自然区分 direct lineage 与 transitive lineage。父 Case 必须证明自己确实消费了哪个子 Result，并验证子结果信封、identity、digest 和适用 contract；它不必在父 Manifest 中复制子 Case 全部代、step 和 Event。若某项父级声明依赖子运行的内部性质，例如“所有训练都使用同一数据切分”，父级便必须要求子 Result 提供相应 attestation 或继续解析子 Manifest。展开深度由当前 claim 决定，不由“尽可能多保存”决定。

保留策略同样属于 lineage 语义。旧 Event 可以压缩，中间 Snapshot 可以淘汰，远端 Manifest 也可能迁移；但压缩记录必须保留摘要算法、被覆盖范围、稳定 digest 与新的解析位置。若最后一份必要见证已经不可取得，系统仍可保留“曾引用过某对象”的历史，却不能继续声称该关系可验证。此时应降低 evidence level 或显式标记 lineage incomplete，而不是用仍然存在的 UUID 假装证据仍然完整。

---

## 14.7 缓存键与随机种子：必须从身份和语义派生
...
## 14.8 Stale handle：引用只有在提交时序中才具有“当前”含义
...
到这里，第十三章的状态与反馈终于有了坐标：UnknownState 有语义内容与实例身份，Candidate 有 Representation 解释版本，Feedback 有 evaluation/attempt 归属，权威状态有逻辑 revision，Result 有 run 与 lineage，Snapshot handle 只有经过 commit 才能成为 current ref。下一章因此可以准确区分 Context、Snapshot、Artifact、Event 与 Manifest：它们不再只是五种保存格式，而是分别承载不同时间尺度、可变性、所有权和证据责任的信息载体。

本章关于身份分层、逻辑阶段和版本正交属于 **I：状态与证据闭包的不变量**；Project run/task/manifest、ResourceContext lineage、状态指纹、decision trace 与更新后提交路径提供 **S：源码证据**；统一 Candidate/Evaluation/Attempt identity、身份派生随机流、lineage cut 以及跨 Store 的 CAS/fencing 仍有 **D：设计收口**。最终判断可以浓缩为一句话：**同一个值可以在不同运行中出现很多次，语义等价可以允许复用；只有身份、时间和版本能够定位当前事实，lineage cut 又能说明本层证明了什么、从哪里继续展开时，框架才有资格说“这条反馈属于这个状态，这个引用就是当前提交”。**

# 第一卷·第十六章　控制对象：资源、预算、截止时间、取消与错误

第十五章把运行信息分配给了 Context、Snapshot、Artifact、Event 与 Manifest。于是一个更深的问题随之出现：放入这些载体的内容，是否都只是等待组件读取的数据？答案是否定的。population snapshot 描述某个提交点的状态，model artifact 描述可以交付的产物；但“这个 Case 最多还能发起多少次评估”“这个子任务是否仍有权使用两条线程”“父运行取消以后这个分支还能不能提交结果”，并不是普通事实描述。它们会决定某个动作是否允许发生，也会让仍在运行的对象失去原有权利。
...
## 16.2 资源不是“机器上有什么”，而是“本次运行获准使用什么”
...
但是，ResourceContext 和 Lease 管理的是逻辑使用权，不是硬件本体。Project 发放“一块 GPU”的 grant，并不表示框架拥有显卡显存回收算法；Case 取得两条线程，也不表示它应在结束时销毁共享 ThreadPoolExecutor。实际执行仍由 Python runtime、操作系统调度器、设备驱动和具体 Backend 完成。框架负责的是让这些底层能力只在获准范围内被使用，并在当前任务结束后关闭或转交自己拥有的义务。

资源 ownership 可以用一组有限规则判断：

```text
created + exclusive      当前 Case 创建且独占：Case 负责及时 close/shutdown
borrowed                 从调用者借入：按合同归还引用，不得擅自关闭本体
shared + authority-owned Project/L0 共享：归还额度或 handle，只有 authority 能关闭全局资源
leased / reserved        向签发者 release/settle，不能只丢弃本地对象
remote / external        请求 cancel/finish/detach，并记录 acknowledged、still_running 或 unknown
committed evidence       Snapshot/Artifact/Event：交给 retention policy，不属于 teardown 垃圾
ephemeral local          普通局部对象：委托语言运行时或操作系统回收
```

这里的 delegated cleanup 与逃避责任不同。框架可以把物理回收委托给 Backend、进程、容器或操作系统，但必须能够指出委托边界。例如，独占子进程被确认退出以后，本地地址空间和文件描述符的物理回收可以以操作系统为 trust root；一个长寿命 Python worker 内的数据库 session 则不能一直等到进程退出，因为它会阻塞连接池复用；远程 Provider 若不支持取消，父 Case 最多只能关闭本地 commit 权、标记任务 detached/still running，并诚实报告残余成本与副作用风险。

只有当一项对象会活过当前调用或进程、被多个 Case 共享、必须及时归还才能让其他任务继续、携带预算/租约/事务/提交权限，或者会在外部世界留下不能靠本进程退出撤销的副作用时，它才需要进入框架级资源协议。其他对象应尽量依赖成熟的底层生命周期原语。统一框架的价值是协调边界，不是把垃圾回收、进程回收、驱动内存管理和数据库实现再写一遍。
...
stateDiagram-v2
    ...
    Active --> Releasing: 当前边界结束使用
    Releasing --> Released: 自有义务关闭或已确认转交
    Releasing --> ResidualRisk: 外部收尾未确认
...
图中的 Released 是框架责任状态，不是对全部物理介质的微观证明。它表示当前 owner 已关闭独占句柄、归还逻辑权利，或把底层回收转交给合同允许的 authority 并得到足够确认。ResidualRisk 则承认另一种现实：父运行已经不能继续等待或控制外部工作，但尚未确认远端停止。把这两种情况都写成 released，会把控制平面的愿望冒充成底层事实。
...
资源控制的目标因此不是尽量填满设备，也不是声称框架亲自回收了一切，而是使每一次执行都能回答：谁申请、谁批准、批准多少、实际采用什么、ownership 属于谁、何时失效、由谁关闭或转交义务、物理回收委托给哪一层，以及旧持有者为何不能再写。只有完成这条因果链，资源才从配置项变成运行边界。

---

## 16.3 预算是一套结算协议，计数器只是其中一种观测
...
## 16.4 Deadline 决定最迟时刻，timeout 只描述一次等待
...
## 16.5 取消是一种协作协议，Fence 才负责拒绝晚到副作用
...
最后，teardown 不是“只有成功才做”的 hook，也不是对所有可见对象统一调用 `close()` 的垃圾回收器。错误 owner 在决定 Result 之前，应通过 `finally` 进入 teardown；teardown 再依据 ownership contract 关闭当前 Case 独占的 session，归还 lease 与 reservation，注销本边界创建的回调或临时 namespace，请求外部任务收尾，并记录没有得到确认的 cleanup error 或 residual risk。它不能关闭 Project/L0 注入的共享 pool，不能销毁借入连接的本体，不能删除已经提交的 Snapshot、Artifact 与 Event，也不必替 Python 和操作系统回收普通局部对象。

当前 nsgablack run loop 在 finally 中调用 teardown 并再将 running 置 false **S**；这项源码事实证明生命周期能够进入统一收尾入口，不自动证明每个具体资源的 owner 判断都正确，也不证明远程或驱动层已经完成物理回收。teardown 自己失败时不能覆盖原始错误，而应成为附属 cleanup error；若底层回收已经委托给进程/Backend 且合同允许，记录委托与确认即可，不应为了“框架亲自释放”再增加一层重复实现。下一章会把这一规则放进完整生命周期代数，因为错误所有权只有和 setup、ownership acquisition、commit、finish、teardown 与 delegated cleanup 的顺序一起看，才能确定哪一层真正拥有结束一次运行的资格。

---

## 16.7 重试不是“再调用一次”，而是受约束的新 attempt
...
第四个边界是 commit。Adapter/Trainer 的权威状态已经确定，系统在 fence 仍有效的前提下写 Snapshot、更新 Context ref、追加 Event、生成或登记 Artifact，并最终形成 Result/Manifest。取消或 lease 过期后的晚到参与者在这里被拒绝。第十五章讨论的五类信息载体，正是在这一边界接收控制对象产生的事实与引用。

commit 还必须关闭本次结果的证明切面。它不是在结束时“尽量多写一些日志”，而是把即将公开的 claim 与有限见证绑定：哪份输入和子 Result 被实际消费，哪个状态 revision 被选择，哪些预算与资源事实已经结算，采用哪项 provenance policy，哪些上游只通过 lineage ref继续展开，哪些外部事实被接受为 trust root，哪些 assumption 仍未由本次运行证明。只有这些关系与结果一起提交，Result 才不会在离开进程后失去自己的证明范围。

这项要求不会让 commit 递归提交全部祖先。子 Case 的内部历史由子 Manifest 负责，父级只验证并固定 child result envelope、digest、contract 和 child Manifest ref；外部数据与 Provider 通过稳定 Artifact/attestation ref进入。commit fence 判断当前参与者是否有权写入，provenance boundary 则判断这次写入有权声称什么。前者防止旧 attempt 篡改权威状态，后者防止证据不足的结果获得过强解释。

如果必要见证在 commit 时不可用，系统不能只因为数值结果已经算出就维持原 claim。它可以失败提交、形成 degraded Result、降低 evidence level、缩小可交付声明，或将未决关系显式登记为 unresolved assumption；但不能把“以后也许能找到”当作当前证据。相反，非必要的远祖记录无需为了追求无限溯源而阻塞提交。证明义务由公开 claim 和 policy 决定，而不是由可以继续追问多少层决定。
...
flowchart LR
    ...
    E --> F{"Fence 仍有效？"}
    F -- "是" --> G["Commit<br/>State + Provenance Boundary"]
...
- requested、granted、resolved 和 observed resources 是否一致，fallback 由谁批准；ownership 是 created、borrowed、shared、leased 还是 external，结束时采用 close、return、release、delegate 还是 residual risk；
...
- commit 支持哪些 claim，采用哪项 provenance policy，直接见证、上游 refs、trust roots 与 unresolved assumptions 分别是什么；
...
本章最终建立的不是一份参数清单，而是一组运行资格：资源回答“可以在哪里、以多大并发执行”，ownership 回答“谁有权关闭、归还或委托哪项义务”，预算回答“还允许付出多少成本”，deadline 回答“最迟何时仍可推进”，取消回答“继续执行的意图是否已被撤销”，fence 回答“旧参与者是否仍有资格提交”，错误回答“未履约发生在哪里、由谁决定后果”，retry 则必须在全部资格重新成立后才能创建下一次 attempt。它们沿父子关系只会收窄，不会凭空扩张；已经发生的成本与错误只会留下新事实，不会被状态回滚抹除。语言运行时、操作系统、容器、驱动和 Backend 继续完成它们擅长的物理生命周期，框架只协调跨边界的权利、义务与证据。

有了这组控制对象，下一步才能严谨回答“一次运行究竟允许发生哪些事件”。setup 何时取得、借入或共享资源，generation/step 在哪里检查 stop 与 cancel，evaluate hook 位于预算 dispatch 的哪一侧，Adapter update 后何时提交权威 Snapshot，commit 怎样同时关闭状态写入权和有限证明义务，错误由哪个公共边界分发，finish 与 teardown 怎样按 ownership 关闭、归还、委托或报告未决责任——这些都不再是任意回调顺序，而会构成下一章的生命周期代数。

...
默认 projector 只消费 `best_objectives/best_feedback`，不会自动把模型 Artifact 带回外层；本卷第一章的通用 Case 信封已经闭合 transport-safe output 与命名 `DataRef`，但专用 Trainer Bridge 若仍只做目标投影，就不会自动获得完整子 Case 的 lineage、预算和 Artifact 交付。[S/I]

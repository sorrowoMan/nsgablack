...
这两个 Case 各自仍可独立运行。`forecast_trial` 独立运行时接收一个明确 TrialSpec fixture 与相同数据 Artifact，使用自己的 canonical builder，并把训练语义投影到统一 Solver 生命周期；被外层调用时，TrialSpec 来自候选 codec，资源来自父 grant 派生，结果通过 projector 返回。外层不构造 mlblack Problem，不访问 LearningSolver 的内部 history，也不持有模型对象。

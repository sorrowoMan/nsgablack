...
设想一个模型校准任务。它独立运行时会读取一份版本明确的数据，申请计算资源，初始化模型，执行若干轮训练，保存 checkpoint，最后交付模型 Artifact、验证指标和运行报告。后来，一个外层结构搜索任务需要反复改变模型结构，并把每次校准后的指标作为候选反馈。最容易写出的代码，是在外层 `evaluate()` 里直接 import 内层 Trainer，临时构造几个对象，然后调用 `fit()`。数值可能正确返回，程序也可能跑完，但原来属于校准任务的输入身份、资源授权、插件生命周期、失败记录、Artifact 提交和恢复入口，已经悄悄消失了。它不再是“那个可独立运行的校准任务”，而变成了一段只在外层函数内部成立的私有代码。
...
轻量 surface 适合另一类工作：输入和输出已经由父 Case 的语义覆盖，调用本身没有独立资源与恢复要求，不产生需要跨运行保存的 Artifact，失败也明确属于父任务当前阶段。例如，Representation 中一次纯 encode/decode、一个无状态 objective 函数，或 Trainer 内部一次不会独立发布的 metric 计算，都不应升级成 Case。它们由父 Case 的生命周期、预算和错误边界管理。

两者之间还有一种重要的“受控短路”形态：子语义本身完整，但父任务只需要它的一个正式评估面。当前 `mlblack.integrations.nsgablack_trainer_evaluator.NsgablackTrainerEvaluator` 就属于这种桥接。[S] 它不会让 nsgablack 外层直接了解 Trainer、Provider 的私有构造细节，而是：

```text
外层候选
  → 转换为 UnknownState，并附外层 individual/generation identity
  → 从父 ResourceContext 派生 training 子 context
  → 由 trainer_factory 创建全新 Trainer
  → 调用 Trainer.evaluate(candidate, resource_context, max_steps)
  → 获得原生 TrainerResult
  → 投影为 nsgablack 所需 objectives 与 violation
```

这里创建“全新 Trainer”不是性能上的偶然选择，而是状态所有权要求。Trainer 拥有可变 population、best state、history、adapter state 与 backend session；多个候选若共享一个 Trainer 实例，前一个候选留下的状态会污染后一个候选，并行时还会产生竞态。`BlankTrainer.evaluate()` 会把候选标准化为 `UnknownState`，保存输入候选快照，把它作为种子 population 注入 Adapter，然后进入正式 `fit()` 生命周期。[S] 因此这不是把训练简化成一个 loss 函数，而是用一个受控入口运行完整 Trainer 语义，再把结果投影给外层。

但这条 Bridge 仍不等于“通用完整子 Case runner”。它当前没有为每次内层训练创建独立 Project Manifest，也没有使用通用 `CaseRunRequest/CaseRunResult` 记录显式父子 lineage；资源派生由 Bridge 完成，结果只投影成目标与约束。[S/D] 因而它适合“每个外层候选把一次训练当作昂贵评估”的场景，前提是内层模型不需要被当作独立交付物恢复和发布。如果业务要求保留每个 trial 的模型 Artifact、单独重试某个 trial、从 trial checkpoint 恢复，或审计每个 trial 的设备与成本，那么仅返回 objectives/violation 已经不够，应把 trial 提升为完整子 Case，或者由 Project 显式展开为多个 Case。

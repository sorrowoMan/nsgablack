...
当前 mlblack 的 population snapshot v2 同时保存 `candidates/objectives/constraints` 与 `evaluated_candidates/evaluated_objectives/evaluated_constraints`，并记录 `feedback_aligned`；ComposableTrainer 在 Adapter update 后解析权威 population，再进行等价对齐并提交快照。当前 nsgablack 的 ComposableSolver 也在 Adapter update 后优先读取 Adapter 权威 population，验证 population、objectives 与 violations 的 N×M/N 关系，再写入代级快照。这些都是源码事实 **S**，表明两侧正在围绕同一不变量收口：**提交点必须位于策略更新之后，但已评估事实不能因权威状态变化而被伪造。**

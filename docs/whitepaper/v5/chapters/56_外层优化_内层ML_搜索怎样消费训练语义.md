...
第 55 章把组合的最小单位固定为 Case，并说明了为什么“能够 import 并调用”远远弱于“能够组合”。现在可以进入第一个真正的跨语义方向：外层优化反复提出候选，每一个候选都触发一次机器学习任务，学习结果再变成外层搜索的反馈。
...
## 56.1 “把训练当作评估”并不等于“把训练压成一个 loss”
...
## 56.2 候选必须先变成 TrialSpec，不能直接穿透成 Trainer 内部字段
...
解码后的 overrides 再交给内层 Case 的 canonical builder。外层不 import 内层 Problem、OptimizerAdapter 或 Provider 私有类，只依赖正式 builder surface：
...
当前 `NsgablackTrainerEvaluator` 已经让 `trainer_factory` 同时接收 `UnknownState` 与评估 context，正是为了让 Case 层完成这种 override 装配，而不让 nsgablack 知道 mlblack 私有组件。[S] 但当前实现还有一个非常具体的适用边界：Bridge 在 factory 返回 Trainer 以后，会再次把同一个外层候选传给 `trainer.evaluate(candidate, ...)`；`BlankTrainer.evaluate()` 又会把它保存为输入快照，并作为种子 population 注入内层 Adapter。[S]
...
## 56.3 公平比较首先由数据、fidelity 与随机性决定
...
当前 `NsgablackTrainerEvaluator` 构造时接收一个固定 `max_steps`，并将它传给每次 `Trainer.evaluate()`。[S] 这为“所有候选使用相同内层步数”的最小公平路径提供了基础，却没有原生表达候选特定 fidelity、晋级 lineage 或 successive-halving 计划。[D] `trainer_factory` 即使在 overrides 中写入另一个最大步数，Bridge 最终传入的 `max_steps` 仍会控制 `BlankTrainer.evaluate()` 的 `fit()` 调用。因此，训练预算搜索需要扩展正式 Bridge 合同，而不是在 factory 里悄悄设置一个可能不生效的字段。
...
## 56.4 TrainerResult 必须经过语义投影，不能把内部 loss 随手拿出来
...
当前默认 `project_trainer_result()` 的语义很清楚：优先读取 `TrainerResult.best_objectives`，若为空则读取 `best_feedback.objectives`；约束从 `best_feedback.constraints` 取得，并把所有正值部分求和为单个 violation。[S] 这遵循 `g(x) <= 0` 为满足约束的约定。没有 objectives 或目标数组为空时会明确报错，而不是返回零。
...
## 56.5 预算、设备和并行必须联合设计，否则试验越多越不可信
...
当前共享 Budget Authority 已经能够通过 `ResourceContext.metadata` 被 Case 侧重建；nsgablack 也已经将它用于 evaluation 预算。[S] 但 `mlblack BlankTrainer` 当前没有像 Solver 一样自动创建并消费一个 `training_steps` 的 `BudgetAccount`，`NsgablackTrainerEvaluator` 也只传递资源上下文与固定 `max_steps`。[S] 因此“一个外层 Project 硬限制所有嵌套训练总步数”仍然是待闭合能力，而不是仅配置 `budgets={"training_steps": ...}` 就已经自动成立。[D]

资源方面，Bridge 会从外层 Solver 的有效 `ResourceContext` 派生 scope 为 training 的子 context，可选地收窄线程，并把它传给 Trainer。[S] `BlankTrainer.set_resource_context()` 会重建 compute backend session、关闭旧 session，并刷新本地 L0 pool，[S] 所以资源声明不会只停留在审计字段中。Trainer 内部候选并行默认关闭；只有显式开启、资源线程数大于一，而且 Problem、Representation 和启用 Plugin 都明确声明线程安全时才会建立 Pool 并并行评估。[S]
...
## 56.6 缓存与 warm start 的价值来自血缘，不来自“少跑了一次”
...
但缓存 key 若只使用候选向量，就会产生比重复计算更危险的错误。数据更新、split 修正、训练步数变化、指标 schema 变化、混合精度变化或代码 bug 修复以后，相同向量不再代表同一次试验。一个合格 cache record 至少应包含第 56.3 节定义的 trial fingerprint、Result schema、状态 `completed`、Artifact checksum 和产生它的 run lineage。只有完整成功并完成提交的记录才能命中；`running`、`failed` 或 Artifact 缺失的记录不能被当成结果。
...
当前 mlblack 已经有规范化 Trainer state 签名，`UnknownState` 也有覆盖 values 与 metadata 的 fingerprint；Snapshot/Artifact 能保存状态引用。[S] 这些是实现缓存和 warm start 血缘的基础。但当前 `NsgablackTrainerEvaluator` 没有内建 cache lookup、trial fingerprint、checkpoint mapper 或 Artifact retention policy，也没有把命中状态投影回外层上下文。[S] 因此跨框架缓存应先在 Case/Bridge 协议中定义，不能在 Solver 的 `problem.evaluate()` 里放一个按 `tuple(x)` 索引的全局字典后就称为可恢复缓存。
...
## 56.7 从一个时间序列搜索 Case 看完整反馈闭环与失败语义
...
```python
from mlblack.integrations import NsgablackTrainerEvaluator
...
def make_trial_trainer(candidate, context):
    trial = decode_trial(candidate.values)
    return build_trial(
        resource_context=context["resource_context"],
        component_overrides={
...
inner_evaluator = NsgablackTrainerEvaluator(
    make_trial_trainer,
    max_steps=20,
    child_threads=1,
    namespace_suffix="forecast_trial",
    result_projector=project_forecast_trial,
)
```

随后，外层 Problem 通过正式 `inner_runtime_evaluator` surface 持有这个 Bridge。nsgablack 的统一评估 helper 在 fallback 路径中调用它，对返回 shape 做验证，再与外层 Problem 的显式约束合并。[S] 这不是把 Bridge 注册成通用 `EvaluationProvider`：当前 `EvaluationMediator` provider 协议要求 `can_handle_individual/evaluate_individual` 等方法，而 `NsgablackTrainerEvaluator` 暴露的是 `can_handle/evaluate`；它当前对应的是 Problem 的 inner runtime surface。[S] 文档和装配代码必须使用真实入口，不能因为两者都“负责评估”就混用。
...
sequenceDiagram
    ...
    participant F as "trainer_factory / canonical builder"
    participant T as "内层 Trainer"
    ...
    B->>B: 构造 UnknownState 与 child ResourceContext
    B->>F: factory(candidate, child context)
    F-->>B: 返回全新 Trainer
    B->>T: evaluate(seed state, max_steps)
    T->>A: 写入输入与权威状态快照
...
## 56.8 当前 Bridge 已经连接了两种语义，但还没有替项目做完所有决定

回到源码，可以把现状分成已经存在的路径与仍需补齐的合同。

已经存在的部分很明确。`mlblack.integrations.nsgablack_trainer_evaluator` 提供正式的外层优化到内层 Trainer Bridge；它为每个调用创建带外层 individual/generation metadata 的 `UnknownState`，从 Solver 或 context 获取父 `ResourceContext`，派生 training 子 context，要求 factory 返回支持 `evaluate()` 的全新 Trainer，并严格要求结果为 `TrainerResult`。[S] 默认 projector 会产生 objectives 数组与基于正约束部分求和的 violation。[S]

`BlankTrainer.evaluate()` 会同步有效资源、保存输入候选快照、设置种子 population，并进入 `fit()`；`fit()` 负责 setup、Plugin 开始、协作式完成条件、step 循环、异常分发、teardown 与最终 Result。[S] 资源改变时 compute backend session 会重建，内部并行默认关闭且受显式线程安全声明约束。[S] nsgablack 的评估 helper 则把 inner runtime 调用纳入统一预算 dispatch、shape 验证、Plugin hook 与外层 constraint 合并。[S]

同时，以下边界尚不能写成“框架已完整支持”。

当前 Bridge 没有把 outer TrialSpec 与 inner seed state 分成两个 mapper，因而只天然覆盖两种状态语义相容的场景；纯超参数搜索需要正式的 `trial_spec_mapper / initial_state_factory` 或完整子 Case request。[D]
...
子 namespace 只包含配置后缀与 individual id，没有 generation、candidate fingerprint、attempt 和 run token；并行 trial 的总资源与 GPU 排他性仍需 Project/L0 scheduler 闭合。[D]

当前 Bridge 没有内建数据 Artifact/split identity、trial fingerprint、结果缓存、warm-start lineage 或模型 Artifact retention。这些应由跨框架 Case 先声明，再逐步下沉为共享协议，不能由 nsgablack core 硬编码 mlblack 数据与 Trainer 细节。[D]

默认 projector 只消费 `best_objectives/best_feedback`，不会自动把模型 Artifact 带回外层；第 55 章的通用 Case 信封已经闭合 transport-safe output 与命名 `DataRef`，但专用 Trainer Bridge 若仍只做目标投影，就不会自动获得完整子 Case 的 lineage、预算和 Artifact 交付。[S/I]

在当前仓库中，Bridge 类已经进入正式 integrations 导出面，但未发现针对 `NsgablackTrainerEvaluator` 的聚焦测试或标准 Case 对它的直接装配引用。[S] 因此本章没有把它标成 [T] 或 [R] 能力。要升级证据等级，至少需要候选到 Trainer 的合同测试、资源与预算集成测试、失败 trial 测试、缓存/warm-start identity 测试，以及一个按标准 Project/Case 脚手架运行并产生 Result、Snapshot、Artifact 与 Manifest 的跨框架案例。

这些缺口并不否定当前架构方向，反而说明 Bridge 的正确职责已经可辨认：它不是另一个 Trainer，也不是外层 Problem 的业务代码仓库；它把一个有身份的搜索候选解释成训练调用，把父授权单调派生给内层，再把原生 TrainerResult 投影成优化反馈。数据公平、预算计量、Artifact 保留与错误分类则必须由围绕它的正式 Case 合同补齐。

至此，组合方向只完成了一半：优化已经能够把学习任务视为昂贵评估。但统一框架不能假定所有组合都是“Solver 在外、Trainer 在内”。下一章会把方向反过来：当学习任务为了生成标签、计算决策损失、执行结构化预测或获得物理反馈而调用一个内层优化问题时，Trainer 应怎样消费 Solver 语义，又为什么不能在自己的 `step()` 中私建资源池和第二个 Project。

...
- **多 Case 并行/阶段协作**：由 Project / Case / L0 substrate 负责 stage、group、依赖、fanout 和资源授权
...
- **深度（Depth）**：外层标准 Case 在 `Problem.evaluate()`、stage runner 或 project runner 中调用内层标准 Case。内层可以是 nsgablack solver、mlblack trainer、数值 solver provider 或仿真 backend，但都必须通过正式 scaffold surface 暴露。
- **广度（Breadth）**：同一层的多个 solver / trainer / mixed Case 由 Project runner 编排；同一个 Case 内的多搜索策略由 Adapter / StrategyRouterAdapter 编排。
- **资源边界**：Project L0 发放 `ResourceContext`，Case 只声明需求、消费 grant 并输出 audit。

推荐阅读：
- `docs/user_guide/DEPTH_BREADTH_PATTERNS.md`
...
- 标准 Project / Case / Scaffold 示例
- 鲁棒性评估示例（MonteCarlo + DP）
- 生产调度案例
- GPU + Ray + MySQL 一体能力说明：`docs/user_guide/GPU_RAY_MYSQL_STACK_DEMO.md`
...
补充入口（深度/广度）：
- 嵌套编排标准：`docs/standard_scaffold_tutorial/07_nested_orchestration_standard.md`

...
**渐进（Progressive）**
- **从原型到生产**：先用 BlankSolverBase 快速验证想法，再抽取可复用模块
...
| 求解器类型 | 适用场景 | 特点 |
|-----------|---------|------|
| `BlackBoxSolverNSGAII` | 标准多目标优化 | 完整 NSGA-II 实现，即插即用 |
| `BlankSolverBase` | 自定义算法实验 | 最小框架，完全自定义流程 |
...
┌─────────────────────────────────────────────────────────────┐
│                     求解器层（Solver）                        │
│  BlackBoxSolverNSGAII  │  BlankSolverBase  │  ComposableSolver│
└─────────────────────────────────────────────────────────────┘
                              ↓
┌─────────────────────────────────────────────────────────────┐
│                   适配器层（Adapter）[可选]                   │
...
| 全新算法流程 | BlankSolverBase + Plugin | 完全自定义，快速实验 |
...
- ❌ 直接用 BlackBoxSolverNSGAII？否，流程完全不同
- ✅ 用 BlankSolverBase + Plugin（原型）→ AlgorithmAdapter（沉淀）
...
- core/solver.py：NSGA-II 底座（BlackBoxSolverNSGAII）。
- core/blank_solver.py：空白底座（BlankSolverBase，流程由 Plugin/子类驱动）。
...
solver = BlankSolverBase(problem, plugins=[MyIdeaPlugin()])
...
# 偏置系统会自动融合这些策略
solver = BlackBoxSolverNSGAII(problem, bias_modules=bias_manager)
```
...
class ParallelSolver(ParallelEvaluationMixin, BlackBoxSolverNSGAII):
    pass
```
...
class MyCustomBias(Bias):
    ...
    Examples:
        ...
        >>> solver = BlackBoxSolverNSGAII(problem, bias_modules=[bias])
    """
```
...
def benchmark_solver(solver_config, n_runs=10):
    ...
    for i in range(n_runs):
        solver = BlackBoxSolverNSGAII(**solver_config)
        result = solver.run()
...
class OldSolver:
    ...
    def __init__(self):
        warnings.warn(
            "OldSolver 已废弃，请使用 BlackBoxSolverNSGAII",
            DeprecationWarning,
            stacklevel=2
        )
```
...
problem = MyProblem()
nsga2_result = BlackBoxSolverNSGAII(problem).run()

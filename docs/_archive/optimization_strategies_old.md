...
```python
from nsgablack.utils.headless import run_headless_single_objective
...
bias_module.add(CallableBias(name="constraint", func=constraint_func, weight=10.0, mode="penalty"))  # 约束惩罚权重
...
bias_module = BiasModule()
# constraint_func 需返回 {"penalty": value}
bias_module.add(CallableBias(name="constraint_strict", func=constraint_func, weight=20.0, mode="penalty"))  # 严格处理约束
solver.bias_module = bias_module
...
```python
注意：历史上的 `nsgablack.solvers.monte_carlo` 已从仓库清理（降低维护成本；如需追溯请查看 git 历史）。当前文档以“框架统一抽象（Problem/Pipeline/Bias/Adapter/Plugin）”为准，不再把 Monte Carlo 优化器作为主线入口。
...
if has_constraints:
    ...
    bias_module = BiasModule()
    # constraint_func 需返回 {"penalty": value}
bias_module.add(CallableBias(name="constraint", func=constraint_func, weight=10.0, mode="penalty"))
    solver.bias_module = bias_module

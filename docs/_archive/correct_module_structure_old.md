...
- 代理/并行/实验口径/协同调度等能力：建议走 Plugin/Wiring（能力层），不污染 solver 底座
...
1) 底座保持纯净：`core/` 不内置高级能力；能力通过 Adapter/Plugin/Bias/Representation/Wiring 接入  
2) 策略逻辑优先 Adapter 化：`propose/update` 是最小闭环  
3) 约束优先分层：硬约束放 repair/可行解构造，软约束放 bias  
4) 必配组合必须提供 Wiring：用 `utils/wiring/attach_*` 固化事实标准，避免漏配  

...
- 元调参实验器（AI/元启发，后置）
  - 目标：做“任意组件权重调节实验器”，估计在约束预算下的边际效用
  - 范围：Bias/Pipeline/Adapter/Plugin 的参数统一调节，不只 bias 权重
  - 路线：先规则化 sweep + 多次重复统计，再逐步引入 AI 策略（bandit/BO/RL）
  - 产出：参数敏感度、推荐区间、冲突参数对、稳定性报告
  - 说明：该能力工程复杂度高，放在 v1 收尾后推进

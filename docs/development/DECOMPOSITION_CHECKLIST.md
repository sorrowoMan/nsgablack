...
## 6. 改进必须回答的问题（研究方法论检查表）

建议你在每次“改进”时逐条回答（不要求一上来全做完，但要尽量覆盖）：

1) 你的改进在组件层面是什么？（新 Bias / 新 Mutation(pipeline) / 新 Adapter / 新 Plugin？）
2) 它解决了什么具体问题？（现有组件的什么缺陷？）
3) 在固定其他组件的条件下，单独验证这个改进有效吗？
4) 改进的参数敏感度如何？是否依赖精细调参？
5) 在哪些问题类型上有效？哪些无效？为什么？
6) 与现有类似组件相比，优势在哪里？
7) 计算开销增加了吗？增加是否值得？

这份清单的目的不是“增加负担”，而是把“可验证的改进”变成可操作的步骤。

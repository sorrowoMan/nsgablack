...
这份文档描述一种**偏置的工程类别**：它本质上是“策略层（policy）”，需要上游能力层提供统计信号，因此通常应与 Plugin/Suite 配套使用。
...
- **解耦但需要配方**：为了避免“用户只开偏置却没有信号 → 看起来没效果”，需要提供权威组合（suite）
...
- 声明 `recommended_plugins: list[str]`（或者提供 suite）
...
## 4. Suite：把“必须组合”变成权威入口

当一个能力必须“成套使用”才有意义时，推荐提供 `utils/suites/*` 的权威组合函数：
...
- `utils/suites/monte_carlo_robustness.py`
...
3. 最后用 Suite 固化“权威组合”与默认值（避免示例/参数发散）

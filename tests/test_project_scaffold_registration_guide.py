...
def test_project_doctor_accepts_declared_optional_context_reads(tmp_path):
    adapter_dir = tmp_path / "adapter"
    adapter_dir.mkdir(parents=True)
    source = (
        "class DemoAdapter:\n"
        "    context_requires = ()\n"
        "    context_optional = ('resource_context', 'resource')\n"
        "    context_provides = ()\n"
        "    context_mutates = ()\n"
        "    context_cache = ()\n"
        "    context_notes = 'ok'\n"
        "    def run(self, context):\n"
        "        return context.get('resource_context', context.get('resource', {}))\n"
    )
    (adapter_dir / "demo_adapter.py").write_text(source, encoding="utf-8")

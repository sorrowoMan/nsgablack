...
from nsgablack.plugins.evaluation.gpu_evaluation_template import GpuEvaluationTemplatePlugin
...
def test_gpu_eval_template_returns_none_when_backend_unavailable(monkeypatch):
    plugin = GpuEvaluationTemplatePlugin()
    ...
    out = plugin.evaluate_population(solver, pop)
    assert out is None

...
def test_surrogate_evaluation_plugin_reduces_true_evals_for_composable_solver():
    ...
    class RandomBatchAdapter(AlgorithmAdapter):
        ...
        def propose(self, control, context):
            ...
            return [np.random.uniform(lows, highs, size=control.dimension) for _ in range(self.n_candidates)]

        def update(self, control, candidates, feedback, context):
            _ = (control, candidates, feedback, context)

    problem = ExpensiveSphere(dim=6)
    control = ComposableSolver(problem=problem, adapter=RandomBatchAdapter(n_candidates=30))
    control.max_steps = 5

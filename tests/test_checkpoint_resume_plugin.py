...
def test_checkpoint_uses_solver_declared_stateful_components(sample_problem) -> None:
    from nsgablack.plugins import CheckpointResumePlugin
    ...
    source_provider = _ProviderState(7)
    source_components = source.checkpoint_components
    source.checkpoint_components = lambda: {
        "evaluation_provider": source_provider,
        **source_components(),
    }
    writer = CheckpointResumePlugin()
    writer.attach(source)
    payload = writer._build_payload(solver=source, reason="component-state")

    target = _build_composable_solver(sample_problem)
    target_provider = _ProviderState(0)
    target_components = target.checkpoint_components
    target.checkpoint_components = lambda: {
        "evaluation_provider": target_provider,
        **target_components(),
    }

...
class TSP2OptAdapter(AlgorithmAdapter):  # 2-opt 局部搜索适配器
    ...
    context_requires = ("population", "objectives")  # 依赖种群和目标
    ...
    def propose(self, solver, context):  # 生成候选
        pop = np.asarray(context["population"], dtype=int)  # 当前种群
    ...
    def update(self, solver, context):  # 更新阶段
        ...
        pop = np.asarray(context["population"], dtype=int)  # 最新种群
        objs = np.asarray(context["objectives"], dtype=float)  # 最新目标

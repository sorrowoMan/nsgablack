# -*- coding: utf-8 -*-
"""Static execution graph helpers for project runtime planning."""
...
def build_execution_lifecycle_graph(solver, *, run_id: str = "planned") -> ExecutionGraph:
    """Build a lifecycle-oriented graph.

    Unlike the assembly graph, this view is ordered by runtime lifecycle:
    init -> population init -> generation loop -> finish.
    """
...
def build_execution_plan_graph(solver, *, run_id: str = "planned") -> ExecutionGraph:
    """Build a pre-run tree of the current solver assembly.

    This graph is intentionally static: it explains what is mounted and where
    resource decisions will apply. Runtime measurements can later be attached
    to the same node ids.
    """
    ...
    return ExecutionGraph(root=root)
...
def _pipeline_node(pipeline: object | None) -> ExecutionGraphNode:
    ...
    children = []
    for attr in ("initializer", "mutator", "repair", "encoder"):
        children.append(_object_node(f"pipeline:{attr}", getattr(pipeline, attr, None)))
    return ExecutionGraphNode(
        node_id="pipeline",

"""
局部优化偏置库 - Local Optimization Biases

提供基于梯度和无梯度的局部优化偏置实现。
这些偏置可以引导优化过程进行更精细的局部搜索。

主要实现：
- GradientDescentBias: 动量梯度下降
- NewtonMethodBias: 牛顿法（带阻尼）
- LineSearchBias: Armijo线搜索
- TrustRegionBias: 信赖域方法
- NelderMeadBias: Nelder-Mead单纯形法
"""
    ...
    def apply(self, x: np.ndarray, eval_func: Callable[[np.ndarray], float],
        ...
        if self.adaptive_lr and len(self.improvement_history) > 5:
            ...
            if np.mean(recent_improvements) < 0:
                self.adaptive_factor *= 0.95  # 减小学习率
            else:
                self.adaptive_factor *= 1.02  # 稍微增大学习率
        ...
        # 评估改进方向
        if context.best_solution is not None:
            direction_to_best = context.best_solution - x
            if self.nesterov:
            ...
            return self.weight * alignment
        else:
            # 没有参考点时，返回梯度下降的潜力
            return -self.weight * np.linalg.norm(gradient) * self.learning_rate * self.adaptive_factor

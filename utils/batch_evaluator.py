...
class BatchEvaluator:
    ...
    def _evaluate_constraints_batch(
        ...
        try:
            ...
            return np.zeros((chunk.shape[0], 0), dtype=float)
    ...
    def _evaluate_serial(
        ...
        for i in range(pop_size):
            ...
            try:
                cons = problem.evaluate_constraints(population[i])
                cons_arr = np.asarray(cons, dtype=float).flatten()
                violation = float(np.sum(np.maximum(cons_arr, 0.0))) if cons_arr.size else 0.0
            except Exception:

...
def import_bias():
    ...
    # Try to import bias components
    bias_components = [
        ('bias_base', ['BaseBias', 'AlgorithmicBias', 'DomainBias', 'OptimizationContext']),
        ('bias_library_algorithmic', ['DiversityBias', 'ConvergenceBias', 'ExplorationBias']),
        ('bias_library_domain', ['ConstraintBias', 'PreferenceBias', 'EngineeringDesignBias']),
        ('bias', ['BiasModule'])
    ]

    for module_name, classes in bias_components:
        try:
            module = safe_import(f'..bias.{module_name}')
            if module:
                for class_name in classes:
                    if hasattr(module, class_name):
                        bias_modules[class_name] = getattr(module, class_name)
        except ImportError:
            continue

...
from ..parallel.evaluator import ParallelEvaluator, SmartEvaluatorSelector
...
class ParallelEvaluationMixin:
    ...
    def evaluate_population_parallel(
        ...
        # 执行并行评估
        result = self.parallel_evaluator.evaluate_population(
        ...
        # 与串行路径对齐：evaluation_count 与 ignore_constraint_violation_when_bias
        try:
            pop_size = int(getattr(population, "shape", [len(population)])[0])
        except Exception:
            pop_size = None

        if pop_size is not None and hasattr(self, 'evaluation_count'):
            try:
                self.evaluation_count += pop_size
            except Exception:
                pass

        if getattr(self, 'enable_bias', False) and getattr(self, 'ignore_constraint_violation_when_bias', False):
            if isinstance(result, dict) and 'constraint_violations' in result:
                result['constraint_violations'] = np.zeros_like(np.asarray(result['constraint_violations'], dtype=float))
            elif isinstance(result, tuple) and len(result) == 2:
                objectives, violations = result
                violations = np.zeros_like(np.asarray(violations, dtype=float))
                result = (objectives, violations)
